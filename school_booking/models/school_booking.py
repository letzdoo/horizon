...
class SchoolCalendar(models.Model):
    ...
    year_id = fields.Many2one(
        "school.year", string="Year", default=lambda self: self.env.user.current_year_id
    )
    from_date = fields.Date("From Date")
    to_date = fields.Date("From Date")
    leave_ids = fields.One2many(
        "school.calendar.leave", "calendar_id", "Leaves", help=""
    )
...
class SchoolLeave(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner",
        string="Partner",
        help="If set, the leave is only for the specified partner.",
    )
    asset_id = fields.Many2one(
        "school.asset", string="Asset", help="If set, the leave is only for the asset."
    )
    category_id = fields.Many2one(
        "school.asset.category",
        string="Category",
        help="If set, the leave is only for the asset of this category.",
    )
...
class Event(models.Model):
    ...
    def _check_room_quota(self):
        for rec in self:
            ...
            _logger.info(
                "Check constraints _check_room_quota for %s on record %s"
                % (self.env.uid, rec.id)
            )
            ...
            with _calendar_event_lock:
                if rec.room_id:
                    ...
                    domain = [
                        ("room_id", "=", int(rec.room_id.id)),
                        ("start", "<", fields.Datetime.to_string(rec.stop)),
                        ("stop", ">", fields.Datetime.to_string(rec.start)),
                    ]
                    conflicts_count = (
                    ...
                    student_event = self.env.ref(
                        "school_booking.school_student_event_type"
                    )
                    ...
                    if student_event in rec.categ_ids:
                        ...
                        if dt.minute != 0 and dt.minute != 30:
                            raise ValidationError(
                                _(
                                    "Invalid booking, please use standard time for booking."
                                )
                            )
                        ...
                        if (
                            now.hour < 19
                            and fields.Datetime.from_string(rec.start).date()
                            != now.date()
                        ):
                            raise ValidationError(
                                _("You cannot book for the next day before 19h00.")
                            )
                        ...
                        if (
                            ...
                            raise ValidationError(
                                _("You can book only the next day (after 19h00).")
                            )
                        ...
                        duration_list = self.env["calendar.event"].read_group(
                            [
                                ...
                                ("room_id", "!=", False),
                                ("categ_ids", "in", student_event.id),
                                ("start", ">=", fields.Datetime.to_string(event_day)),
                                (
                                    "start",
                                    "<=",
                        ...
                        for duration in duration_list:
                            if duration["duration"] and duration["duration"] > 2:
                                raise ValidationError(
                                    _(
                                        "You cannot book the room %s more than two hours per day"
                                    )
                                    % (duration.get("room_id", "")[1])
                                )
                        ...
                        for duration in duration_list:
                            if duration["duration"] and duration["duration"] > 6:
                                raise ValidationError(
                                    _(
                                        "You cannot book more than six hours per day - %s"
                                    )
                                    % duration["start:day"]
                                )
                        ...
                        for duration in duration_list:
                            if duration["duration"] > 4:
                                raise ValidationError(
                                    _(
                                        "You cannot book more than four hours in advance per day - %s"
                                    )
                                    % duration["start:day"]
                                )
    ...
    def archive_old_events(self, *args, **kwargs):
        ...
        domain = [
            ("stop", "<", timeout_ago.strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
            ("recurrency", "=", False),
        ]
        self.sudo().search(domain).write({"active": False})
        _logger.info("Archive reccuring event")
        domain = [
            ("final_date", "<", timeout_ago.strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
            ("recurrency", "=", True),
        ]
        self.sudo().search(domain).write({"active": False})
