...
class CourseDocumentation(models.Model):
    ...
    course_group_id = fields.Many2one(
        "school.course_group", related="course_id.course_group_id"
    )
    ...
    def _onchange_course_id(self):
        self.write({"course_ids": [(4, self.course_id.id)]})

    def unpublish(self):
        return self.write({"state": "draft"})

    def publish(self):
        current = self.env["school.course_documentation"].search(
            [["course_id", "=", self.course_id.id], ["state", "=", "published"]]
        )
        if current:
    ...
    partner_id = fields.Many2one(
        "res.partner", related="author_id.partner_id", store=True
    )

    staff_ids = fields.Many2many(
        "res.partner",
        "school_desc_res_partner_rel",
        "desc_id",
        "res_partner_id",
        string="Teachers",
        domain=[("teacher", "=", 1)],
    ...
    schedule = fields.Selection(
        [("Q1", "Q1"), ("Q2", "Q2"), ("Q1Q2", "Q1 & Q2"), ("O", "Other")],
        string="Schedule",
        required=True,
        default="Q1",
    )
...
class Course(models.Model):
    ...
    all_documentation_ids = fields.One2many(
        "school.course_documentation", "course_id", string="All Documentations"
    )  # TODO affiner ce champs

    all_documentation_count = fields.Integer(
        string="Documentation Count", compute="_compute_count"
    )
    ...
    def _search_documentation_id(self, operator, value):
        if value == "missing":
            ...
            current_program_ids = self.env["school.program"].search(
            ...
            recs = (
                self.env["school.course_documentation"]
