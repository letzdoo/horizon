...
class IndividualProgram(models.Model):
    ...
    def _check_cycle(self):
        for rec in self:
            ...
            if len(duplicates) > 0 :
                all_cgs = rec.all_ind_course_group_ids.filtered(lambda ic: ic.state != '7_failed') +  rec.valuated_course_group_ids
                duplate_cgs = all_cgs.filtered(lambda ic: ic.source_course_group_id.uid in duplicates)               
                raise ValidationError("Cannot have duplicated UE in a program : %s.\n\n Please check following UEI : %s" % (self.env['school.course_group'].browse(duplicates).mapped('uid'),duplate_cgs.mapped('uid')))
...
class IndividualCourseGroup(models.Model):
    ...
    def compute_average_results(self, force = False):
        for rec in self : # .filtered(lambda r: force or r.state in ['7_failed','5_progress']) :
            _logger.debug('Trigger "compute_average_results" on Course Group %s' % rec.uid)
    ...
    def compute_first_session_results(self, force = False):
        for rec in self : # .filtered(lambda r: force or r.state in ['7_failed','5_progress']) :
            _logger.debug('Trigger "compute_first_session_results" on Course Group %s' % rec.uid)
    ...
    def compute_second_session_results(self, force = False):
        for rec in self : # .filtered(lambda r: force or r.state in ['7_failed','5_progress']) :
            _logger.debug('Trigger "compute_second_session_results" on Course Group %s' % rec.uid)
    ...
    def compute_final_results(self, force = False):
        for rec in self : # .filtered(lambda r: force or r.state in ['7_failed','5_progress']) :
            _logger.debug('Trigger "compute_final_results" on Course Group %s' % rec.uid)
