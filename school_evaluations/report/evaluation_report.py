...
class ReportEvaluationByTeacher(models.AbstractModel):
    ...
    def render_html(self, data):
        ...
        if data["teacher_ids"]:
            ...
            teacher_ids = (
                self.env["school.individual_course_proxy"]
                .search([["year_id", "=", data["year_id"] or current_year_id.id]])
                .mapped("teacher_id")
        ...
        for teacher_id in teacher_ids:
            source_course_ids = (
                ...
                .search(
                    [
                        ["year_id", "=", data["year_id"] or current_year_id.id],
                        ["teacher_id", "=", teacher_id.id],
                    ]
                )
                .mapped("source_course_id")
                .sorted(key=lambda r: r.name)
            )
            courses = []
            for source_course_id in source_course_ids:
                individual_courses_ids = (
                    self.env["school.individual_course"]
                    .search(
                        [
                            ["year_id", "=", data["year_id"] or current_year_id.id],
                            ["teacher_id", "=", teacher_id.id],
                            ["source_course_id", "=", source_course_id.id],
                        ]
                    )
                    .sorted(key=lambda r: r.student_id.name)
                )
                courses.append(
                    {
        ...
        return self.env["report"].render(
            "school_evaluations.report_evaluation_by_teacher_content", docargs
        )
