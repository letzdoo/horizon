    ...
    def _get_options (self, programs, segment):
        ...
        if (segment == 0):
            for program in programs:
                option = {'uri' : actual + '/' + program.year_name, 'name' : program.year_name}
                if (option not in options):
                    options.append(option)
        if (segment == 1):
            for program in programs:
                option = {'uri' : actual + '/' + program.domain_slug, 'name' : program.domain_name}
                if (option not in options):
                    options.append(option)
        elif (segment == 2):
            for program in programs:
                option = {'uri' : actual + '/' + program.track_slug, 'name' : program.track_name}
                if (option not in options):
                    options.append(option)            
        elif (segment == 3):
            for program in programs:
                option = {'uri' : actual + '/' + program.speciality_slug, 'name' : program.speciality_name}
                if (option not in options):
                    options.append(option)            
        elif (segment == 4):
            for program in programs:
                option = {'uri' : actual + '/' + program.cycle_grade_slug, 'name' : program.cycle_grade}
                if (option not in options):
                    options.append(option) 
        elif (segment == 5):
            for program in programs:
            ...
            for program in programs:
                if (program.specialization_slug and program.specialization):
                    option = {'uri' : actual + '/' + program.specialization_slug, 'name' : program.specialization}
                else: 
                    option = '-'
                if (option not in options):
                    options.append(option)
